...
;(function($){
	...
	updateHotspotSelector = function(id, id2){
		id = id || 'hotspotSelector';
				...
				var myCallBacks = {
					...
					onLoad: function(){ // onSpinPreloadEnd
						...
						$.fn.axZm.loadHotspotsFromJsFile(hotspotFileToLoad, false, function(){
							...
							$('#jsFileName').val(getHotspotJsFile ? getHotspotJsFile.replace('.js','') : '');
	...
	toggleNaviBar = function(a){
		if ($('#testCustomNavi').css('display') == 'block'){
			$('#zoomCustomNavi, #testCustomNavi').css('display', 'none');
			$(a).html('activate');
		}else{
			$('#zoomCustomNavi, #testCustomNavi').css('display', '');
			$(a).html('deactivate');
		}
	};
	...
	$.aZhSpotEd = {
		...
		updateHotspotSelector: function(a, b){updateHotspotSelector(a, b)},
