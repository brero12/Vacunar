...
* Version: 4.0.3 Patch: 2013-02-25
* Date: 2013-02-24
...
;(function($) {
	...
	jQuery.fn.zoomImage = function(orgImage, linkObj, spin){
		...
		// just do not forget to trigger jQuery.fn.axZm(); after ajax request is completed (must have a callback for it)
			...
			function axZmSetFancyBoxLabel(){
				if (jQuery.axZm && !jQuery.axZm.spinMod && typeof jQuery.axZmLabels === "object"){
					var picTitle = jQuery.axZmLabels[jQuery.axZm.zoomID];
					if (picTitle && jQuery.axZmFancyLabel){
						jQuery('#fancy_title').css('display', 'block');
						jQuery('#fancy_title_main').html('<div>'+picTitle+'</div>');
					}else{
						jQuery('#fancy_title_main').html('<div>-</div>');
						jQuery('#fancy_title').css('display', 'none');
					}
